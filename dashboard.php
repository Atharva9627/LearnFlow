...
<head>
    ...
    <style>
        .dashboard-grid { display: grid; grid-template-columns: 1fr 300px; gap: 20px; padding: 20px; }
        @media (max-width: 900px) { .dashboard-grid { grid-template-columns: 1fr; } }
        .feed-item { background: var(--bg-card); border: 1px solid var(--glass-border); padding: 20px; border-radius: 15px; margin-bottom: 20px; transition: 0.3s; }
        .feed-item:hover { transform: translateY(-3px); border-color: var(--primary); }
        .type-tag { font-size: 0.7rem; text-transform: uppercase; padding: 3px 8px; border-radius: 5px; font-weight: bold; margin-right: 10px; }
        ...
        .post-form input, .post-form select, .post-form textarea { width: 100%; padding: 12px; margin-bottom: 10px; background: rgba(0,0,0,0.2); border: 1px solid var(--glass-border); color: white; border-radius: 8px; }
    </style>
</head>
<body>
            ...
            <nav class="nav-menu">
                ...
                <?php if ($current_class_id): ?>
                    ...
                    <?php if ($userRole === 'teacher'): ?>
                        <a href="create_quiz.php?class_id=<?= $current_class_id ?>" class="nav-item"><i data-lucide="plus-square"></i> Give Quiz</a>
                    ...
                    <?php else: ?>
                        <a href="take_quiz.php?class_id=<?= $current_class_id ?>" class="nav-item"><i data-lucide="pen-tool"></i> Take Quiz</a>
                        <a href="assignments.php?class_id=<?= $current_class_id ?>" class="nav-item"><i data-lucide="clipboard-list"></i> My Assignments</a>
                        <a href="leaderboard.php?class_id=<?= $current_class_id ?>" class="nav-item"><i data-lucide="trophy"></i> Leaderboard</a>
                    <?php endif; ?>
                <?php endif; ?>
                ...
                <div class="feed-section">
                        ...
                        <div class="feed-item">
                            <div style="display: flex; justify-content: space-between; align-items: flex-start;">
                                <div>
                                    ...
                                    <span style="color: var(--text-dim); font-size: 0.8rem;"><?php echo $item['class_name']; ?> • By <?php echo $item['author']; ?></span>
                    ...
                    <?php endforeach; else: ?>
                        <div class="card" style="text-align: center; padding: 40px;">
                            ...
                            <p style="margin-top: 15px; color: var(--text-dim);">No updates yet.</p>
                        </div>
                    <?php endif; ?>
                </div>
                ...
                <div class="actions-section">
                    <?php if ($userRole === 'teacher'): ?>
                        <div class="card post-form">
                            <h3><i data-lucide="megaphone"></i> Post Announcement</h3>
                            <p style="font-size: 0.8rem; color: var(--text-dim); margin-bottom: 15px;">Message your students.</p>
                            <form action="backend/post_material.php" method="POST">
                                ...
                                <select name="class_id" required>
                                    ...
                                    while($class = $cStmt->fetch()) {
                                        $sel = ($current_class_id == $class['id']) ? 'selected' : '';
                                        echo "<option value='{$class['id']}' $sel>{$class['class_name']}</option>";
                                    }
                                    ?>
                                </select>
                                <input type="text" name="title" placeholder="Topic" required>
                                <textarea name="content" placeholder="Share something with your class..." rows="4" required></textarea>
                    ...
                    <?php else: ?>
                        <div class="card">
                            ...
                            <form action="backend/join_class.php" method="POST" class="post-form">
                                <input type="text" name="invite_code" placeholder="Code" maxlength="6" required>
